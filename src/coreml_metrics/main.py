...
def main():
  ...
  parser.add_argument('--model-input-path', type=str, dest='model_input_path',
                      default='output/models/yolov5-iOS.mlmodel', help='path to coreml model')
  parser.add_argument('--image-folder', type=str, dest='image_folder',
                      default='data/images', help='path to image root folder')
  parser.add_argument('--label-folder', type=str, dest='label_folder', default='data/labels',
                      help='path to label root folder (folder needs to mirror directory structure of the image folder)')
  parser.add_argument('--metrics_output-directory', type=str, dest='metrics_output_directory',
                      default='output/metrics', help='path to metrics output folder (will be created if it does not exist)')
  opt = parser.parse_args()

  Path(opt.metrics_output_directory).mkdir(parents=True, exist_ok=True)

  allBoundingBoxes = queryFolders(
      opt.model_input_path, opt.image_folder, opt.label_folder, opt.metrics_output_directory)
...
def queryFolders(model, imageFolder, labelsFolder, outputDirectory):
  ...
  for subImageFolder in os.scandir(imageFolder):
    ...
    if subImageFolder.is_dir():
      subBoundingBoxes = queryFolders(
          model, subImageFolder.path, f'{labelsFolder}/{subImageFolder.name}', outputDirectory)
      allBoundingBoxes.addBoundingBoxes(subBoundingBoxes)

  boundingBoxes = analyseCurrentDir(
      model, imageFolder, labelsFolder, outputDirectory)
  if boundingBoxes:
  ...
  metricsOutputFolder = imageFolder.replace(
      'data', outputDirectory) + '/metrics'
...
def analyseCurrentDir(model, imageFolder, labelsFolder, outputDirectory):
  ...
  detectionOutputFolder = imageFolder.replace(
      'data', outputDirectory) + '/detections'
...
def detectCoreML(modelPath, imageEntries, outputFolder):
  ...
  for imageEntry in imageEntries:
    ...
    out_dict = model.predict(
        {'image': inputImage, 'iouThreshold': IOU_THRESHOLD_MODEL, 'confidenceThreshold': CONFIDENCE_THRESHOLD})
    ...
    with open(outFilePath, 'w') as outFile:
      for coordinates, confidence in zip(out_dict['coordinates'], out_dict['confidence']):
        ...
        outFile.write('{:d} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f}\n'.format(
            labelMax, coordinates[0], coordinates[1], coordinates[2], coordinates[3], confidence[labelMax]))
  ...
  for labelFileEntry in os.scandir(labelFolder):
    ...
    with open(labelFileEntry.path, 'r') as labelFile:
      for labelLine in labelFile:
        ...
        if isGroundTruth:
          bb = BoundingBox(imageName, LABELS[labelNumbers[0]], float(labelNumbers[1]), float(labelNumbers[2]), float(labelNumbers[3]), float(
              labelNumbers[4]), CoordinatesType.Relative, (IMAGE_SIZE, IMAGE_SIZE), BBType.GroundTruth, format=BBFormat.XYWH)
        else:
          bb = BoundingBox(imageName, LABELS[labelNumbers[0]], float(labelNumbers[1]), float(labelNumbers[2]), float(labelNumbers[3]), float(
              labelNumbers[4]), CoordinatesType.Relative, (IMAGE_SIZE, IMAGE_SIZE), BBType.Detected, float(labelNumbers[5]), format=BBFormat.XYWH)
