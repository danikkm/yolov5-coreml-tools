...
def includeNumberOfLayers(model, numberOfLayers):
  model.model = torch.nn.Sequential(
      *[model.model[i] for i in range(numberOfLayers)])
...
def fixModel(modelSpec):
  ...
  for layer in modelSpec.neuralNetwork.layers:
    if layer.WhichOneof('layer') == 'pooling':
      layer.pooling.kernelSize[:] = [
          kernelSizes[poolingCount], kernelSizes[poolingCount]]  # [5, 5]

      # 2
      layer.pooling.valid.paddingAmounts.borderAmounts[0].startEdgeSize = kernelSizes[poolingCount] // 2
      # 2
      layer.pooling.valid.paddingAmounts.borderAmounts[0].endEdgeSize = kernelSizes[poolingCount] // 2
      # 2
      layer.pooling.valid.paddingAmounts.borderAmounts[1].startEdgeSize = kernelSizes[poolingCount] // 2
      # 2
      layer.pooling.valid.paddingAmounts.borderAmounts[1].endEdgeSize = kernelSizes[poolingCount] // 2
...
def fixModel2(modelSpecOld, modelSpec):
  ...
  for i in range(8):
    modelSpec.neuralNetwork.layers.insert(
        i,  modelSpecOld.neuralNetwork.layers[i])
...
class SPP(torch.nn.Module):
  ...
  def __init__(self, c1, c2, k=(9,)):
    ...
    self.m = torch.nn.ModuleList(
        [torch.nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
